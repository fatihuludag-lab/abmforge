...
def test_list_templates_contains_network() -> None:
...
def test_create_network_project(tmp_path: Path) -> None:
    project = create_project(tmp_path / "network-study", template="network")
...
def test_scaffolded_network_baseline_runs(tmp_path: Path, monkeypatch) -> None:
    project = create_project(tmp_path / "network-run", template="network")
...
def test_scaffolded_network_experiment_and_report_run(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project = create_project(tmp_path / "network-experiment", template="network")
    ...
    text = text.replace(
        "threshold_mean: [0.25, 0.35, 0.45]",
        "threshold_mean: [0.25, 0.45]",
    )
