...
def test_list_templates_contains_resource() -> None:
...
def test_create_resource_project(tmp_path: Path) -> None:
    project = create_project(tmp_path / "resource-study", template="resource")
...
def test_scaffolded_resource_baseline_runs(tmp_path: Path, monkeypatch) -> None:
    project = create_project(tmp_path / "resource-run", template="resource")
...
def test_scaffolded_resource_experiment_and_report_run(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project = create_project(tmp_path / "resource-experiment", template="resource")
    ...
    text = text.replace(
        "resource_regrowth: [0.15, 0.35, 0.60]",
        "resource_regrowth: [0.15, 0.60]",
    )
