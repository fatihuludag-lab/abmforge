...
def test_list_templates_contains_epidemic() -> None:
...
def test_create_epidemic_project(tmp_path: Path) -> None:
    project = create_project(tmp_path / "epidemic-study", template="epidemic")
...
def test_scaffolded_epidemic_baseline_runs(tmp_path: Path, monkeypatch) -> None:
    project = create_project(tmp_path / "epidemic-run", template="epidemic")
...
def test_scaffolded_epidemic_experiment_and_report_run(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project = create_project(tmp_path / "epidemic-experiment", template="epidemic")
    ...
    text = text.replace(
        "infection_probability: [0.03, 0.06, 0.09]",
        "infection_probability: [0.03, 0.09]",
    )
